...
def create_all_locations(world: GoblinWorld) -> None:
    create_regular_locations(world)
    create_events(world)


def create_regular_locations(world: GoblinWorld) -> None:
    ...
    tutorial_start.add_locations(tutorial_start_locations, GoblinLocation)

    tutorial_key_locations = get_location_names_with_ids(
        ["Tutorial: Hat"]
    )
    tutorial_key.add_locations(tutorial_key_locations, GoblinLocation)
    ...
    forest_start.add_locations(forest_start_locations, GoblinLocation)
    ...
    forest_bronze.add_locations(forest_bronze_locations, GoblinLocation)

    forest_silver_locations = get_location_names_with_ids(
        ["Forest Village: Gold Key"]
    )
    forest_silver.add_locations(forest_silver_locations, GoblinLocation)

    forest_gold_locations = get_location_names_with_ids(
        ["Forest Village: Terrace Treat"]
    )
    forest_gold.add_locations(forest_gold_locations, GoblinLocation)
    ...
    market_start.add_locations(market_start_locations, GoblinLocation)

    market_hat_locations = get_location_names_with_ids(
        ["Market Town: Viking Hat 1"]
    )
    market_hat.add_locations(market_hat_locations, GoblinLocation)
    ...
    market_bronze.add_locations(market_bronze_locations, GoblinLocation)

    market_silver_locations = get_location_names_with_ids(
        ["Market Town: Gold Key"]
    )
    market_silver.add_locations(market_silver_locations, GoblinLocation)

    market_gold_locations = get_location_names_with_ids(
        ["Market Town: Bedside Bounty"]
    )
    market_gold.add_locations(market_gold_locations, GoblinLocation)
    ...
    downtown_start.add_locations(downtown_start_locations, GoblinLocation)

    downtown_bronze_locations = get_location_names_with_ids(
        ["Downtown: Silver Key", "Downtown: Viking Hat 2"]
    )
    downtown_bronze.add_locations(downtown_bronze_locations, GoblinLocation)
    ...
    downtown_silver.add_locations(downtown_silver_locations, GoblinLocation)

    downtown_gold_locations = get_location_names_with_ids(
        ["Downtown: Wizard Hat 2", "Downtown: Gutter Gourmet", "Downtown: Rampart Ration"]
    )
    downtown_gold.add_locations(downtown_gold_locations, GoblinLocation)

    # Royal Castle:
    castle_start_locations = get_location_names_with_ids(
        ["Royal Castle: Bronze Key"]
    )
    castle_start.add_locations(castle_start_locations, GoblinLocation)

    castle_bronze_only_locations = get_location_names_with_ids(
        ["Royal Castle: Viking Hat 1", "Royal Castle: Rogue's Reward"]
    )
    castle_bronze_only.add_locations(castle_bronze_only_locations, GoblinLocation)
    ...
    castle_bronze.add_locations(castle_bronze_locations, GoblinLocation)
    ...
    castle_silver.add_locations(castle_silver_locations, GoblinLocation)

    castle_gold_locations = get_location_names_with_ids(
        ["Royal Castle: Crown", "Royal Castle: Castle Crumbs",
         "Royal Castle: Bastion Banquet"]
    )
    castle_gold.add_locations(castle_gold_locations, GoblinLocation)
    ...
    tower_start.add_locations(tower_start_locations, GoblinLocation)

    tower_bronze_locations = get_location_names_with_ids(
        ["Clamber Tower: Silver Key"]
    )
    tower_bronze.add_locations(tower_bronze_locations, GoblinLocation)
    ...
    tower_silver.add_locations(tower_silver_locations, GoblinLocation)

    tower_gold_locations = get_location_names_with_ids(
        ["Clamber Tower: Purple Key"]
    )
    tower_gold.add_locations(tower_gold_locations, GoblinLocation)

    tower_purple_locations = get_location_names_with_ids(
        ["Clamber Tower: Teetering Treat"]
    )
    tower_purple.add_locations(tower_purple_locations, GoblinLocation)


def create_events(world: GoblinWorld) -> None:
    ...
    tutorial_key.add_event(
        ...
        location_type=GoblinLocation, item_type=items.GoblinItem
    )
