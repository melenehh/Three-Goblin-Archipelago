...
FSplinePoint USplineComponent::GetSplinePointAt(int32 PointIndex, TEnumAsByte<ESplineCoordinateSpace::Type> CoordinateSpace) const {
    return FSplinePoint{};
}
...
float USplineComponent::GetDistanceAlongSplineAtLocation(const FVector& InLocation, TEnumAsByte<ESplineCoordinateSpace::Type> CoordinateSpace) const {
    return 0.0f;
}
...
void USplineComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
    ...
    DOREPLIFETIME(USplineComponent, SplineCurves);
    DOREPLIFETIME(USplineComponent, ReparamStepsPerSegment);
    DOREPLIFETIME(USplineComponent, bStationaryEndpoints);
    DOREPLIFETIME(USplineComponent, bSplineHasBeenEdited);
    DOREPLIFETIME(USplineComponent, bInputSplinePointsToConstructionScript);
    DOREPLIFETIME(USplineComponent, bClosedLoop);
    DOREPLIFETIME(USplineComponent, bLoopPositionOverride);
    DOREPLIFETIME(USplineComponent, LoopPosition);
    DOREPLIFETIME(USplineComponent, DefaultUpVector);
}
