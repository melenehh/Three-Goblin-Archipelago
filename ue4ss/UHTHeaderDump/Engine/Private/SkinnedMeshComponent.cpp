...
USkinnedMeshComponent::USkinnedMeshComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    ...
    this->bForceUpdateDynamicDataImmediately = false;
...
FTransform USkinnedMeshComponent::GetBoneTransform(FName InBoneName, TEnumAsByte<ERelativeTransformSpace> TransformSpace) const {
    return FTransform{};
}
